...
```{r echo=FALSE}
#Fancy table packages
library(knitr)
library(kableExtra)

data.frame(
  label = c("High median CV coverage",
            "Low alignment percentage",
            "Low total sequences",
            "PCA outlier",
            "Rare"),
  value = c(0,0,0,0,
        nrow(dat.AM.pc$genes)-nrow(dat.AM.pc.abund$genes))) %>% 
kable(align="l", col.names = c("",""),
        caption="AM data cleaning summary") %>% 
  kable_styling(bootstrap_options = "striped", full_width = FALSE) %>% 
  pack_rows("Samples removed", 1,4) %>% 
  pack_rows("Genes removed", 5,5)
```
...
data.frame(
  ...
  value = c(0,0,0,0,
        nrow(dat.MDM.pc$genes)-nrow(dat.MDM.pc.abund$genes))) %>% 
kable(align="l", col.names = c("",""),
        caption="MDM data cleaning summary") %>% 
  kable_styling(bootstrap_options = "striped", full_width = FALSE) %>% 
  pack_rows("Samples removed", 1,4) %>% 
  pack_rows("Genes removed", 5,5)
```
